...
const TestComponent: TestComponentInterface = function TestComponent (props) {
    ...
    return <div className="p-6 max-w-sm bg-white rounded-xl shadow-lg flex items-center space-x-4">
        <div>
            <div className={`text-xl font-medium text-black ${styles.text}`}>{modifiedTitle.value}</div>
            <p className="text-slate-500">{content}</p>
            <button type="button" onClick={modifyTitle}>Modify title!</button>
        </div>
    </div>
}
